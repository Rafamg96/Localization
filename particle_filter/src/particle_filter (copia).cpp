...
int main(int argc, char **argv){
	...
	if (pcl::io::loadPCDFile<pcl::PointXYZRGB> ("/home/rafael/catkin_ws/pcd/prueba2.pcd", *cloud) == -1) //* load the file
  {
...
void obtenerImagenParticula(pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr in_nubemapeo,float x, float y, float angle, cv::Mat &imagen){
	...
	pcl::PointCloud<pcl::PointXYZRGB> bodyFiltered2;
	bodyFiltered2=*in_nubemapeo;
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr bodyFiltered3 (new pcl::PointCloud<pcl::PointXYZRGB>);
	*bodyFiltered3=bodyFiltered2;
	pcl::PointCloud<pcl::PointXYZRGB>::Ptr bodyFiltered (new pcl::PointCloud<pcl::PointXYZRGB>);
	pcl::CropBox<pcl::PointXYZRGB> boxFilter;
	boxFilter.setMin(Eigen::Vector4f(-5, -5, -1, 1.0));
	boxFilter.setMax(Eigen::Vector4f(5,5, 1, 1.0));
	boxFilter.setInputCloud(bodyFiltered3); 
	boxFilter.filter(*bodyFiltered);
